...
class Export_excel_c extends CI_Controller {
    ...
    public function export_sms_trans()
    {
        ...
        $query = $this->db->query("SELECT
              ...
              `created_by`,
              `provider`
            FROM `backend_member`.`sms_transaction` a
    ...
    public function export_purchase_detail()
    {
        ...
        $data = $this->dbutil->csv_from_result($query, $delimiter, $newline);
        force_download('Purchase Details Report.csv', $data);
    }
    ...
    public function export_ewallet_purchase_detail()
    {
        ...
        $query = $this->db->query("SELECT b.refno,CONCAT(b.sysdate,' ',b.systime) AS transdate,
                ...
                WHERE a.account_no = '".$AccountNo."' AND a.trans_type = 'PAY' 
