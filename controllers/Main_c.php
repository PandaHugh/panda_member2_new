...
class main_c extends CI_Controller {
    ...
    public function dashbord()
    {
        ...
        { 
            $total_activation = $this->db->query("SELECT COUNT(AccountNo) AS num FROM member WHERE Active = 1 and Expirydate > CURDATE() ")->row('num');
            $total_member = $this->db->query("SELECT COUNT(AccountNo) AS num FROM member WHERE acc_status <> 'TERMINATE'")->row('num');
            // $total_activation = $this->db->query("SELECT COUNT(a.AccountNo) AS num FROM member as a, set_parameter as b WHERE a.Active = 1 AND a.Expirydate >= CURDATE() and IF(b.terminate_expiry_month <> 0,`Expirydate` <> `Issuedate`, Expirydate)")->row('num');
            // $total_member = $this->db->query("SELECT COUNT(AccountNo) AS num FROM member AS a, set_parameter AS b WHERE IF(b.terminate_expiry_month <> 0, Expirydate >= (CURDATE() - INTERVAL b.terminate_expiry_month MONTH) AND `Expirydate` <> `Issuedate`, Expirydate)")->row('num');
            $total_preactivated = $this->db->query("SELECT count(AccountNo) as num FROM `member` a WHERE a.`Expirydate` = '3000-12-31';")->row('num');
            ...
            $data = array(
                ...
                // 'total_expiry_date_member' => $total_expiry_date_member = $this->db->query("SELECT COUNT(AccountNo) AS num FROM member a, set_parameter AS b WHERE a.`Expirydate` <> a.`Issuedate` AND IF(terminate_expiry_month <> 0,Expirydate < CURDATE() AND Expirydate >= (CURDATE() - INTERVAL terminate_expiry_month MONTH), Expirydate < CURDATE())")->row('num'),
                'total_expiry_date_member' => $total_expiry_date_member = $this->db->query("SELECT COUNT(AccountNo) AS num FROM member a WHERE Expirydate < CURDATE() AND a.`Expirydate` <> a.`Issuedate` AND a.acc_status <> 'TERMINATE'")->row('num'),
        ...
        if($this->session->userdata('loginuser')== true)
        {
            ...
            if($this->input->post('search') == 'General')
            {
                $data['data'] = $this->db->query("SELECT * FROM (
                    ...
                    UNION ALL
                    SELECT b.SupCardNo AS CardNo, b.AccountNo, b.Expirydate, b.ICNo, b.Phonemobile, b.Name FROM member a INNER JOIN membersupcard b ON a.accountno = b.accountno WHERE supcardno LIKE '%$key%' )a1  GROUP BY accountno");
            ...
            if($this->input->post('search') == 'Card')
            {
                ...
                $data['data'] = $this->db->query("SELECT CardNo, AccountNo, Expirydate, ICNo, Phonemobile, `Name` FROM member WHERE CardNo like '%$key%' UNION ALL
                    SELECT b.SupCardNo AS CardNo, b.AccountNo, b.Expirydate, b.ICNo, b.Phonemobile, b.Name FROM member a INNER JOIN membersupcard b ON a.accountno = b.accountno WHERE supcardno LIKE '%$key%' ");
            };

            if($this->input->post('search') == 'Account')
            {
                $data['data'] = $this->db->query("SELECT CardNo, AccountNo, Expirydate, ICNo, Phonemobile, `Name`FROM member WHERE AccountNo LIKE '%$key%' 
                    UNION ALL
                    SELECT b.SupCardNo AS CardNo, b.AccountNo, b.Expirydate, b.ICNo, b.Phonemobile, b.Name FROM member a INNER JOIN membersupcard b ON a.accountno = b.accountno WHERE a.AccountNo LIKE '%$key%' ");
            };

            if($this->input->post('search') == 'Name')
            {
                $data['data'] = $this->db->query("SELECT CardNo, AccountNo, Expirydate, ICNo, Phonemobile, `Name`FROM member WHERE Name like '%$key%' UNION ALL
                    SELECT b.SupCardNo AS CardNo, b.AccountNo, b.Expirydate, b.ICNo, b.Phonemobile, b.Name FROM member a INNER JOIN membersupcard b ON a.accountno = b.accountno WHERE a.Name LIKE '%$key%'");
            };

            if($this->input->post('search') == 'Passport')
            {
                $data['data'] = $this->db->query("SELECT CardNo, AccountNo, Expirydate, ICNo, Phonemobile, `Name`FROM member WHERE PassportNo like '%$key%' ");
            };

            if($this->input->post('search') == 'Ic')
            {
                $data['data'] = $this->db->query("SELECT CardNo, AccountNo, Expirydate, ICNo, Phonemobile, `Name`FROM member WHERE ICNo like '%$key%' 
                    UNION ALL
                    SELECT b.SupCardNo AS CardNo, b.AccountNo, b.Expirydate, b.ICNo, b.Phonemobile, b.Name FROM member a INNER JOIN membersupcard b ON a.accountno = b.accountno WHERE a.ICNo LIKE '%$key%'");
            };

            if($this->input->post('search') == 'Phone')
            {
                $data['data'] = $this->db->query("SELECT CardNo, AccountNo, Expirydate, ICNo, Phonemobile, `Name`FROM member WHERE Phonemobile LIKE '%$key%' 
                    UNION ALL
                    SELECT b.SupCardNo AS CardNo, b.AccountNo, b.Expirydate, b.ICNo, b.Phonemobile, b.Name FROM member a INNER JOIN membersupcard b ON a.accountno = b.accountno WHERE a.Phonemobile LIKE '%$key%' ");
            };

            if($this->input->post('search') == 'Address')
            {
                $data['data'] = $this->db->query("SELECT CardNo, AccountNo, Expirydate, ICNo, Phonemobile, `Name`FROM member WHERE Address1 LIKE '%$key%' 
                    UNION ALL
                    SELECT b.SupCardNo AS CardNo, b.AccountNo, b.Expirydate, b.ICNo, b.Phonemobile, b.Name FROM member a INNER JOIN membersupcard b ON a.accountno = b.accountno WHERE a.Address1 LIKE '%$key%';");
            };
            $data['form_search'] = site_url('Main_c/search');
            $this->template->load('template' , 'home' , $data);
        }
    ...
    public function search_merchant()
    {
        if($this->session->userdata('loginuser')== true)
        {
            ...
            if($this->input->post('search') == 'Card')
            {
                ...
                $data['data'] = $this->db->query("SELECT * FROM member WHERE CardNo like '%$key%' AND CardNo IN (SELECT CardNo FROM member_merchantcard WHERE merchant_id = '".$_SESSION['branch_code']."' )UNION ALL
                    SELECT a.* FROM member a INNER JOIN membersupcard b ON a.accountno = b.accountno WHERE supcardno LIKE '%$key%' ");
            };

            if($this->input->post('search') == 'Account')
            {
                $data['data'] = $this->db->query("SELECT * FROM member WHERE AccountNo like '%$key%' AND CardNo IN (SELECT CardNo FROM member_merchantcard WHERE merchant_id = '".$_SESSION['branch_code']."') UNION ALL
                    SELECT a.* FROM member a INNER JOIN membersupcard b ON a.accountno = b.accountno WHERE a.AccountNo LIKE '%$key%' ");
            };

            if($this->input->post('search') == 'Name')
            {
                $data['data'] = $this->db->query("SELECT * FROM member WHERE Name like '%$key%' AND CardNo IN (SELECT CardNo FROM member_merchantcard WHERE merchant_id = '".$_SESSION['branch_code']."' )UNION ALL
                    SELECT a.* FROM member a INNER JOIN membersupcard b ON a.accountno = b.accountno WHERE a.Name LIKE '%$key%'");
            };

            if($this->input->post('search') == 'Passport')
            {
                $data['data'] = $this->db->query("SELECT * FROM member WHERE PassportNo like '%$key%' AND CardNo IN (SELECT CardNo FROM member_merchantcard WHERE merchant_id = '".$_SESSION['branch_code']."' ");
            };

            if($this->input->post('search') == 'Ic')
            {
                $data['data'] = $this->db->query("SELECT * FROM member WHERE ICNo like '%$key%' 
                    AND CardNo IN (SELECT CardNo FROM member_merchantcard WHERE merchant_id = '".$_SESSION['branch_code']."') UNION ALL
                    SELECT a.* FROM member a INNER JOIN membersupcard b ON a.accountno = b.accountno WHERE a.ICNo LIKE '%$key%'");
            };
            ...
            if($this->input->post('search') == 'Phone')
            {
                $data['data'] = $this->db->query("SELECT * FROM member WHERE Phonemobile LIKE '%$key%' AND CardNo IN (SELECT CardNo FROM member_merchantcard WHERE merchant_id = '".$_SESSION['branch_code']."' )
                    UNION ALL
                    SELECT a.* FROM member a INNER JOIN membersupcard b ON a.accountno = b.accountno WHERE a.Phonemobile LIKE '%$key%' ");
            };
            ...
            if($this->input->post('search') == 'Address')
            {
                $data['data'] = $this->db->query("SELECT * FROM member WHERE Address1 LIKE '%$key%' AND CardNo IN (SELECT CardNo FROM member_merchantcard WHERE merchant_id = '".$_SESSION['branch_code']."')
                    UNION ALL
                    SELECT a.* FROM member a INNER JOIN membersupcard b ON a.accountno = b.accountno WHERE a.Address1 LIKE '%$key%';");
            };
            $data['form_search'] = site_url('Main_c/search_merchant');
    ...
    public function full_details()
    {
        ...
        {  
            ...
            {
                $select_branch = $result1['select_branch'];
            }
            ...
            $data = array(
                ...
                'set_nationality' => $this->db->query("SELECT * FROM set_nationality ORDER BY Nationality ASC "),
                'set_title' => $this->db->query("SELECT * FROM set_title ORDER BY Title ASC"),
                'set_race' => $this->db->query("SELECT * FROM set_race ORDER BY Race ASC"),
                'set_religion' => $this->db->query("SELECT * FROM set_religion ORDER BY Religion ASC"),
                'set_status' => $this->db->query("SELECT * FROM set_status ORDER BY Status ASC"),
                'set_occupation' => $this->db->query("SELECT * FROM set_occupation ORDER BY Occupation ASC"),
                'set_misc' => $this->db->query("SELECT * FROM set_misc ORDER BY misc_name ASC"),
                'set_cardtype' => $this->db->query("SELECT * FROM cardtype ORDER BY Cardtype ASC"),
                ...
                'check_active' => $this->db->query("SELECT COUNT(*) AS active FROM member WHERE AccountNo = '".$_REQUEST['AccountNo']."' AND Active = 1 AND Expirydate <> Issuedate")->row('active'),

                'movement_point_details' => $this->db->query("SELECT *, (Pointsbalance - PointsBF) AS Pointsearn FROM points_movement WHERE AccountNo = '".$_REQUEST['AccountNo']."' ORDER BY `PeriodCode` DESC"),
            ...
            {
                $data['changes'] = $this->db->query("SELECT * FROM mem_ii_trans WHERE AccountNo = '".$result->row('AccountNo')."' ORDER BY created_at DESC");
            }
    ...
    public function supcard_details()
    {
        if($this->session->userdata('loginuser')== true)
        {
            ...
            $data = array(
                ...
                'set_relationship' => $this->db->query("SELECT * FROM set_relationship "),
                'supcard_record' => $this->db->query("SELECT * FROM membersupcard WHERE AccountNo = '".$_REQUEST['AccountNo']."' AND PrincipalCardNo = 'SUPCARD' "),
    ...
    public function purchase_list_server_side()
    {
        $columns = array(
            0 => "TRANS_TYPE",
            1 => "RefNo",
        ...
        {
            $data['search'] = "";
            $output = $this->Member_Model->query_call('Main_c', 'purchase_list_server_side', $data);

            if(isset($output) && $output['message'] == 'success')
            {
                if(isset($output['purchase_list']))
                {
                    $posts = $output['purchase_list'];
                    $totalFiltered = $totalData = $output['num_rows'][0]['num_rows'];
                }
            }
        }
        else 
        {
            ...
            if(isset($output) && $output['message'] == 'success')
            {
                $totalData = $output['num_rows'][0]['num_rows'];

                if(isset($output['purchase_list']))
                {
                    $posts = $output['purchase_list'];
                    $totalFiltered = $output['search_num_rows'][0]['num_rows'];
                }
            }
        }
        ...
        if(!empty($posts))
        {
            foreach ($posts as $post)
            {
                ...
                $nestedData['BizDate'] = $post['BizDate'];
                $nestedData['SysDate'] = $post['SysDate'];
                ...
                {
                    $nestedData['view'] = "<button class='btn btn-xs btn-primary' onclick='trans_child_list(\"".$post['RefNo']."\")'><i class='fa fa-eye'></i></button>";
                }
                else
                {
                    $nestedData['view'] = "<button class='btn btn-xs btn-primary' onclick='point_child_list(\"".$post['RefNo']."\")'><i class='fa fa-eye'></i></button>";
                }
    ...
    public function purchase_child_list_server_side()
    {
        $columns = array(
            0 => "RefNo",
            1 => "PosID",
        ...
        {
            $data['search'] = "";
            $output = $this->Member_Model->query_call('Main_c', 'purchase_child_list_server_side', $data);
        ...
        if(!empty($posts))
        {
            ...
            {
                $nestedData['RefNo'] = $post['RefNo'];
                $nestedData['PosID'] = $post['PosID'];
                $nestedData['Location'] = $post['Location'];
                $nestedData['BizDate'] = $post['BizDate'];
                $nestedData['User'] = $post['User'];
                $nestedData['Line'] = $post['Line'];
                $nestedData['Itemcode'] = $post['Itemcode'];
                $nestedData['Description'] = $post['Description'];
                $nestedData['Point'] = $post['Point'];
                $nestedData['UnitPrice'] = $post['UnitPrice'];
                $nestedData['Qty'] = $post['Qty'];
                $nestedData['Amount'] = $post['Amount'];

                if($post['Itemrefund'] == 0)
                {
                    $nestedData['Itemrefund'] = "<i class='fa fa-close'></i>";
                }
                else
                {
                    $post['Itemrefund'] = "<i class='fa fa-check'></i>";
                }
    ...
    public function point_child_list_server_side()
    {
        $columns = array(
            0 => "REF_NO",
            1 => "TRANS_TYPE",
        ...
        {
            $data['search'] = "";
            $output = $this->Member_Model->query_call('Main_c', 'point_child_list_server_side', $data);

            if(isset($output) && $output['message'] == 'success')
            {
                if(isset($output['point_transaction']))
                {
                    $posts = $output['point_transaction'];
                    $totalFiltered = $totalData = $output['num_rows'][0]['num_rows'];
                }
            }
        }
        ...
        if(!empty($posts))
        {
            foreach ($posts as $post)
            {
                ...
                $nestedData['DESCRIPTION'] = $post['DESCRIPTION'];
                $nestedData['QTY'] = $post['QTY'];
                $nestedData['VALUE_UNIT'] = $post['VALUE_UNIT'];
    ...
    public function update()
    {
        ...
        {  
            if($this->input->post('pass') == 'submit')
            {
                ...
                //ori_log
                $ori = $this->db->query("SELECT * from member where AccountNo = '".$this->input->post('AccountNo')."' ");
                ...
                if($this->db->affected_rows() > 0)
                {
                    ...
                    //upd_log
                    $upd = $this->db->query("SELECT * from member where AccountNo = '".$this->input->post('AccountNo')."' ");
            ...
            else
            {
                ...
                $branch = array();
                $branch_list = $this->Member_Model->query_call('Main_c', 'update', $data);
                ...
                $data = array(
                    ...
                    'set_misc' => $this->db->query("SELECT * FROM set_misc"),
                    ...
                    'movement_point_details' => $this->db->query("SELECT * FROM points_movement WHERE AccountNo = '".$this->input->post('AccountNo')."'"),
                    'cardhistory' => $this->db->query("SELECT cardno, created_at , 'First Card' as status from member where accountno = '".$_REQUEST['AccountNo']."'
                    union all
                    SELECT supcardno as cardno, issuestamp as created_at, 'Replaced' as status from membersupcard where accountno = '".$_REQUEST['AccountNo']."' and principalcardno = 'LOSTCARD'"),
    ...
    public function update_supcard()
    {
        ...
        {  
                ...
                //ori_log
                $ori = $this->db->query("SELECT * from membersupcard where AccountNo = '".$this->input->post('AccountNo')."' ");
                $ori_AccountNo = $ori->row('AccountNo');
                $ori_CardNo = $ori->row('CardNo');
                $ori_ICNo = $ori->row('ICNo');
                ...
                if($this->db->affected_rows() > 0)
                {
                    ...
                    //upd_log
                    $upd = $this->db->query("SELECT * from membersupcard where AccountNo = '".$this->input->post('AccountNo')."' ");
                    $upd_AccountNo = $upd->row('AccountNo');
                    $upd_CardNo = $upd->row('CardNo');
                    $upd_ICNo = $upd->row('ICNo');
                    $upd_Birthdate = $upd->row('Birthdate');
    ...
    public function purchase_details_server_side()
    {
        $columns = array(
            0 => 'refno',
            1 => 'transdate',
        ...
        {
            $data['search'] = "";
            $output = $this->Member_Model->query_call('Main_c', 'purchase_details_server_side', $data);
            
            if(isset($output) && $output['message'] == 'success')
            {
                if(isset($output['purchase_details']))
                {
                    $posts = $output['purchase_details'];
                    $totalFiltered = $totalData = $output['num_rows'][0]['num_rows'];
                }
            }
        }
...
public function update_misc()
    ...
    public function check_point()
    {
        if($this->session->userdata('loginuser')== true)
        {
            ...
            {
                $get_data = $this->db->query("SELECT * FROM member WHERE CardNo = '".$_REQUEST['exist']."' UNION ALL
                    SELECT a.* FROM member a INNER JOIN membersupcard b ON a.accountno = b.accountno WHERE supcardno = '".$_REQUEST['exist']."'");
                $get_point_movement = $this->db->query("SELECT * FROM points_movement WHERE AccountNo = '".$get_data->row('AccountNo')."' order by PeriodCode desc ");
            ...
            {
                $get_point_movement = $this->db->query("SELECT * FROM points_movement WHERE AccountNo = '' order by PeriodCode desc ");
    ...
    {
     // echo 'asdasd';
        $query =  $this->db->query("SELECT a.CardNo, a.AccountNo, a.Expirydate, a.ICNo, a.Phonemobile, a.`Name` , acc_balance from member as a inner join member_wallet as b on a.accountno = b.accountno ");
    ...
    public function ewallet_old()
    {
        if($this->session->userdata('loginuser')== true)
        {
            $check_member = $this->db->query("SELECT a.CardNo, a.AccountNo, a.Expirydate, a.ICNo, a.Phonemobile, a.`Name` from member as a inner join member_wallet as b on a.accountno = b.accountno ");
    ...
    public function ewallet_submit()
    {
        if($this->session->userdata('loginuser')== true)
        {
            ...
            $verify_card = $this->db->query("SELECT CardNo, AccountNo, Expirydate, ICNo, Phonemobile, `Name` from member  where cardno = '$cardno' and active = '1' and Name <> 'NEW'");
            if($verify_card->num_rows() > 0 )
            {
                $check_already_e_wallet = $this->db->query("SELECT CardNo, AccountNo, Active from member_wallet where cardno = '$cardno' and active = '1'");
    ...
    public function ewallet_assign_process()
    {
        if($this->session->userdata('loginuser')== true)
        {
            ...
            $check_details = $this->db->query("SELECT * from member where cardno = '$check_cardno'");
            $accountno = $check_details->row('AccountNo');

            $this->db->query("INSERT INTO member_wallet SELECT '$check_guid' as wallet_guid
    ...
    public function ewallet_topup_submit()
    {
        if($this->session->userdata('loginuser')== true)
        {
            $cardno = $this->input->post("confirm_cardno");
            $check_already_e_wallet = $this->db->query("SELECT CardNo, AccountNo, Active from member_wallet where cardno = '$cardno' and active = '1'");
            $check_member = $this->db->query("SELECT a.CardNo, a.AccountNo, a.Expirydate, a.ICNo, a.Phonemobile, a.`Name` , wallet_name, acc_balance, b.active from member as a inner join member_wallet as b on a.accountno = b.accountno where a.accountno = '".$check_already_e_wallet->row('AccountNo')."'");
    ...
    public function ewallet_topup_process()
    {
        if($this->session->userdata('loginuser')== true)
        {
            ...
            /*echo '';die;*/
            $double_refresh_checking = $this->db->query("SELECT * from member_wallet_trans where trans_guid = '$guid'");
            ...
            {   
                $check_member = $this->db->query("SELECT * from member where AccountNo = '$accountno'");

                $check_wallet = $this->db->query("SELECT * from member_wallet where AccountNo = '$accountno'");
                ...
                $this->db->query("UPDATE member_wallet set acc_balance = '$new_amount' where accountno = '$accountno' and cardno = '$cardno' and active = '1'");

                //print receipt
                if($this->db->affected_rows() > 0)
                {
                    $guid = $_REQUEST['guid'];
                    $main = $this->db->query("SELECT * from member_wallet_trans where trans_guid = '".$_REQUEST['guid']."' ");
                    $child = $this->db->query("SELECT * from member_wallet_trans where trans_guid = '".$_REQUEST['guid']."' ");
                    ...
                    $result = $this->Member_Model->query_call('Main_c', 'ewallet_topup_process');
                    $companyname = "";

                    if(isset($result['companyprofile']))
                    {
                        $companyname = $result['companyprofile'][0]['CompanyName'];
                    }
                    ...
                    $data3 = array(
                        ...
                        'child' => $child,
                        'text' => $text,
                        'Point_before' => $this->db->query("SELECT value_from from user_logs where receiptno = '$refno' and trans_type = 'TOPUP'")->row('value_from'),
    ...
    public function reload_from_dropbox()
    {
        if($this->session->userdata('loginuser') == true && $this->session->userdata('username') != '')
        {
            ...
            $file_path = $this->db->query("SELECT file_path from set_parameter limit 1")->row('file_path');

            $destination_path = $this->db->query("SELECT destination_path from set_parameter limit 1")->row('destination_path');
            $permission = '0777';
            ...
            if($fp = file_get_contents($file_name)) 
                {
                  ...
                  //echo var_dump($var_array);
                    ...
                    {   
                        if($value != '')
                        {
                            if($database != 'backend_member')
                            {
                                $customer = substr($database, strrpos($database, '_', -1));
                                $database_name = $this->db->query('SELECT DISTINCT(a.database_name) AS database_name FROM set_query_parameter AS a'); 

                                foreach($database_name->result() as $d_name)
                                {
                                    $value = str_replace(substr($d_name->database_name, 0, strripos($d_name->database_name, '_')), $d_name->database_name, $value);
                                }
                            }
                        ...
                        {   
                            $this->db->query("UPDATE set_parameter set script_datetime = '$file_gmt'");
                            $this->db->query("UPDATE set_parameter set updated_at = now()");
